<template>
  <div class="app-container">
      ...
      <el-col :span="20" :xs="24">
        <el-form v-show="showSearch" ref="queryForm" :model="queryParams" size="small" :inline="true" label-width="68px">
          ...
          <el-form-item>
            ...
            <el-button isize="small" @click="resetQuery">刷新</el-button>
          </el-form-item>
        </el-form>
        ...
        <el-table v-loading="loading" :data="userList" @selection-change="handleSelectionChange">
          ...
          >
            <template v-if="scope.row.userId !== 1" slot-scope="scope">
              ...
              >删除</el-button>
              <el-dropdown v-hasPermi="['system:user:resetPwd', 'system:user:edit']" size="small" @command="(command) => handleCommand(command, scope.row)">
    ...
    <el-dialog :title="title" :visible.sync="open" width="600px" append-to-body>
      <el-form ref="form" :model="form" :rules="rules" label-width="80px" :disabled="form.userType == '01'">
        <el-row>
          <el-col :span="12">
          ...
          <el-col :span="12">
            <el-form-item label="归属部门" prop="deptId">
              <treeselect v-model="form.deptId" :options="deptOptions" :show-count="true" :disabled="form.userType == '01'" placeholder="请选择归属部门" />
            </el-form-item>
          </el-col>
        </el-row>
        <el-row>
      ...
      <div slot="footer" class="dialog-footer">
        ...
        <el-button v-if="form.userType != '01'" type="primary" @click="submitForm">确 定</el-button>
      </div>
    </el-dialog>
...
<script>
import { listUser, getUser, delUser, addUser, updateUser, resetUserPwd, changeUserStatus, deptTreeSelect, getConfigKey } from '@/api/system/user'
...
export default {
  name: 'User',
  components: { Treeselect, headerTitle },
  data() {
    return {
      // 遮罩层
      loading: true,
  ...
  methods: {
    ...
    getList() {
      this.loading = true
      listUser(this.queryParams, this.dateRange).then(response => {
    ...
    handleUpdate(row) {
      ...
      getUser(userId).then(response => {
        ...
        this.open = true
        this.title = this.form.userType === '01' ? '查看用户' : '修改用户'
        this.form.password = ''
      })
    },
    /** 重置密码按钮操作 */
    ...
    handleAuthRole: function(row) {
      ...
      // this.$router.push('/system/user/' + userId)
    ...
    async handleDelete(row) {
      try {
        ...
        await this.$confirm('是否确认删除用户编号为"' + userIds + '"的数据项？')
        ...
        if (error !== 'cancel') {
          this.$message.error(error.data.msg ? error.data.msg : '删除失败')
        }
      }
    },
    /** 导出按钮操作 */
