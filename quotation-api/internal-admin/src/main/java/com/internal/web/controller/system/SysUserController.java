...
import com.internal.system.service.ISysPostService;
import com.internal.system.service.ISysRoleService;
...
//        ajax.put("roles", SysUser.isAdmin(userId) ? roles : roles.stream().filter(r -> !r.isAdmin()).collect(Collectors.toList()));
     ...
     * 获取指定角色的用户列表
     */
    @ApiOperation(value = "获取指定角色的用户列表", notes = "获取指定角色的用户列表")
    @GetMapping("/roleUserList/{roleId}")
    public TableDataInfo<List<SysUser>> roleUserList(@PathVariable("roleId") Long roleId)
    {
        startPage();
        List<SysUser> list = userService.selectRoleUserList(roleId);
        return getDataTable(list);
    }
