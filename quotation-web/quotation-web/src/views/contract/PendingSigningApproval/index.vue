<template>
  <div>
  <a-card title="待签约审批" :headStyle="{ 'height': '66px' }">
    <template #extra>
    ...
    <a-table :columns="visibleColumns" :data-source="data" :pagination="pagination" row-key="key" :loading="loading"
      ...
      <template #bodyCell="{ column, record, index }">
        ...
        <span v-else-if="column.dataIndex === 'actions'" style="text-align: left;">
          <a-button v-hasPermi="['quote:contract:approval:details']" type="link" @click="handleDetail(record)" class="nomp">详情</a-button>
          <a-button v-hasPermi="['quote:contract:approval:handleLost']" type="link" @click="handleUnqoute('discard', record)" class="nomp">丢单处理</a-button>
          <a-button v-hasPermi="['quote:contract:approval:update']" type="link" @click="handleUpdate(record)" class="nomp">更新签约申请</a-button>
          <a-button v-hasPermi="['quote:contract:approval:approve']" type="link" @click="handleApproval(record)" class="nomp">签约审批</a-button>
        </span>
      </template>
    </a-table>
  </a-card>
  <QuotationApproval :open="isQuotationApprovalVisible" @approval-contractsigning-close="handleQuotationApprovalClose"
    ...
    @contract-apply-close="handleQuotationApprovalClose" :isSummarySheet="isSummarySheet" :opportunity="selectedRecord"
  ...
  <ApproveDetail :open="isModalVisible" :record="selectedRecord" @close="handleClose" @ok="handleOk"
    :isModalVisible="isModalVisible" :isWaitingSign="isWaitingSign"></ApproveDetail>
  </div>
</template>  
...
const loadOpportunities = async () => {
  ...
  try {
    const response = await fetchOpportunities('pageWithSign',params);
...
const handleApproval = (record) => {
  selectedRecord.value = record;
  isSummarySheet.value = true
  componentTitle.value = '签约审批'
  isApprovalOfContractSigning.value = true
  isQuotationApprovalVisible.value = true
};
